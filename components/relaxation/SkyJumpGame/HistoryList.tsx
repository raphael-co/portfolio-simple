...
export default function HistoryList({
  ...
  history: SJRun[];
  labels: { title: string; none: string; clear: string };
  onClear: () => void;
}) {
  return (
    <div className="mt-6 rounded-2xl border p-4 sm:p-5 dark:border-white/10">
      ...
      {history.length === 0 ? (
        ...
        <ul className="mt-3 space-y-2">
          {history.map((r) => (
            ...
            >
              <span className="opacity-80">{r.dateKey}</span>
      ...
      {history.length > 0 && (
        <div className="mt-3">
          <button onClick={onClear} className="text-xs opacity-60 hover:opacity-100 underline">
            {labels.clear}
          </button>
        </div>
      )}
